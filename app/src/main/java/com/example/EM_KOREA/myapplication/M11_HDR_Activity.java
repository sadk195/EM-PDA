...
public class M11_HDR_Activity extends BaseActivity {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if ((event.getAction() == KeyEvent.ACTION_DOWN) && (keyCode == KeyEvent.KEYCODE_ENTER)) {
                    ...
                    if (txt_Scan_st.equals("")) {
                        TGSClass.AlterMessage(getApplicationContext(), "검사요청번호를 입력하여 주시기 바랍니다.");
                        return false;
                    } else {
    ...
    private boolean setScanData(String pJobCD) {
        ...
        if (sJson.equals("[]")) {
            TGSClass.AlterMessage(getApplicationContext(), "검색 된 검사요청번호 결과가 없습니다.");
            return false;
        }

        if (Q_STS.equals("N")) {
            TGSClass.AlterMessage(getApplicationContext(), "검사결과 등록 후 입고처리해주세요.");
            return false;
        } else if (Q_STS.equals("D")) {
            TGSClass.AlterMessage(getApplicationContext(), "검사결과 확정처리 후 입고처리해주세요.");
            sJson = "[]";
            return false;
        }

        if (M_STS.equals("IT")) {
            TGSClass.AlterMessage(getApplicationContext(), "입고완료된 자료가 선택되었습니다.");
            return false;
        }
