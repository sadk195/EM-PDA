        ...
        btn_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (outplace_chk.equals("Y")) {
                    AlertDialog.Builder builder = new AlertDialog.Builder(S16_DTL_Activity.this);
                    builder.setTitle("출하등록")
                            .setMessage("품목을 전부 출하대기장으로\n이동하신 후 출고처리를\n진행해 주시기 바랍니다.")
                            .setPositiveButton("확인", null)
                            .create().show();
                    return;
                } else {
                    if (DN_RQ_DT.getText().toString().equals("") || ((GetComboNUM)cmbBizPartner.getSelectedItem()).getMINOR_CD().equals("")) {
                        TGSClass.AlterMessage(getApplicationContext(), "필수 값을 입력하여 주십시오.");
                        return;
                    }
                    //== 기존 소스를 save() 메서드로 이동 ==//
                    save();
