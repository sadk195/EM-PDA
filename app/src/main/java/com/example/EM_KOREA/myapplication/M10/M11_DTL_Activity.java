package com.example.EM_KOREA.myapplication.M10;
...
public class M11_DTL_Activity extends BaseActivity {
    ...
    private void initializeView() {
        ...
        //== ID 값 바인딩 ==//
        insp_req_no         = (EditText) findViewById(R.id.insp_req_no);        // 검사요청번호
        start_dt            = (EditText) findViewById(R.id.start_dt);           // 검사일자
        item_cd             = (EditText) findViewById(R.id.item_cd);            // 품번
        item_nm             = (EditText) findViewById(R.id.item_nm);            // 품명
        STS                 = (EditText) findViewById(R.id.STS);                // 진행상태
        INSPECTOR_NM        = (EditText) findViewById(R.id.INSPECTOR_NM);       // 검사원
        inspect_good_qty    = (EditText) findViewById(R.id.inspect_good_qty);   // 양품수량
        inspect_bad_qty     = (EditText) findViewById(R.id.inspect_bad_qty);    // 불량수량
        mvmt_rcpt_qty       = (EditText) findViewById(R.id.mvmt_rcpt_qty);      // 요청수량
        blank               = (EditText) findViewById(R.id.blank);              // -빈칸-
        bp_cd               = (EditText) findViewById(R.id.bp_cd);              // 고객사 코드
        bp_nm               = (EditText) findViewById(R.id.bp_nm);              // 고객사명
        MVMT_NO             = (EditText) findViewById(R.id.MVMT_NO);            //
        M_STS               = (EditText) findViewById(R.id.M_STS);              // 검사상태
        INSP_RESULT_NO      = (EditText) findViewById(R.id.INSP_RESULT_NO);     //
        SL_CD_FOR_GOOD      = (EditText) findViewById(R.id.SL_CD_FOR_GOOD);     //
        SL_CD_FOR_DEFECT    = (EditText) findViewById(R.id.SL_CD_FOR_DEFECT);   //
        in_dt               = (EditText) findViewById(R.id.in_dt);              // 입고일자
        save_location       = (EditText) findViewById(R.id.save_location);      // 적치장

        qty                 = (EditText) findViewById(R.id.qty);                // 적치장 총합 재고수량
        top_location        = (EditText) findViewById(R.id.top_location);       // 최다 재고수량이 있는 적치장코드
        ...
        if (!sJson.equals("[]")) {
            try {
                ...
                for (int idx = 0; idx < ja.length(); idx++) {
                    ...
                    start_dt.setText(jObject.getString("MVMT_RCPT_DT"));                // 기간
    ...
    private void save() {
        if (M_STS.getText().toString().equals("IF")) {
            String in_dt_st     = in_dt.getText().toString();
            String MVMT_NO_st   = MVMT_NO.getText().toString();

            fncSet_list_bl("C", in_dt_st, MVMT_NO_st);

            AlertDialog.Builder builder = new AlertDialog.Builder(M11_DTL_Activity.this);
            builder.setTitle("구매입고등록\n(검사완료품 입고등록)")
            ...
            String in_dt_st             = in_dt.getText().toString();       //입고일자
            ...
            if (!insp_req_no_st.equals("") && !INSP_RESULT_NO_st.equals("")) {
                ...
                TGSClass.AlterMessage(getApplicationContext(), "검사의뢰번호 혹은 검사결과번호가 없습니다.");
                return;
            }
