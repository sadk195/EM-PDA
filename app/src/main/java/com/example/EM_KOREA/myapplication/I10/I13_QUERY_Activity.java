        ...
        rdo_grp.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, int chkId) {
                ...
                if (sJsonHDR.equals("[]") || sJsonHDR.isEmpty()) {
                    TGSClass.AlterMessage(getApplicationContext(), "선택 된 품목이 없습니다.");
                    return;
                } else {
                    dbQuery_LIST();
                }
...
//        listview.addHeaderView(head);
    ...
    private void start(String str_item_cd) {
        ...
        if (sJsonHDR.isEmpty() || sJsonHDR.equals("[]")) {
            ...
            TGSClass.AlterMessage(getApplicationContext(), "존재하지 않는 품번입니다.", 1000);
            return;
        } else {
            try {
    ...
    public class DBQueryList extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                TGSClass.AlterMessage(I13_QUERY_Activity.this, ex.getMessage());
            } catch (Exception e1) {
                TGSClass.AlterMessage(I13_QUERY_Activity.this, e1.getMessage());
            }
        }
    }
...
//                                        TGSClass.AlterMessage(getApplicationContext(), position + "번째 item");
                                    }
                                });

                                TGSClass.AlterMessage(I13_QUERY_Activity.this, ja.length() + " 건 조회되었습니다.");
                            }
                        }
                    } catch (JSONException ex) {
                        TGSClass.AlterMessage(I13_QUERY_Activity.this, ex.getMessage());
