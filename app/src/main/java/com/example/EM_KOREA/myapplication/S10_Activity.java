...
public class S10_Activity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_s10_sub_menu);
            ...
            public void onClick(View v) {
                switch (v.getId()) {
                    ...
                    case R.id.btn_shipment_request_status:          // 1. 출하요청현황
                        /*
                        if (start_grant("S11")) {
                        Intent intent = TGSClass.ChangeView(getPackageName(), S11_QUERY_Activity.class.getSimpleName());
                    ...
                    case R.id.btn_shipment_registration:            // 2. 출하등록
                        if (start_grant("S12")) {
                            Intent intent = TGSClass.ChangeView(getPackageName(), S12_HDR_Activity.class.getSimpleName());
                    ...
                    case R.id.btn_shipment_status:                  // 3. 출하현황
                        /* 개발중...
                        if (start_grant("S13")) {
                            Intent intent = TGSClass.ChangeView(getPackageName(), S13_HDR_Activity.class.getSimpleName());
                    ...
                    case R.id.btn_shipment_location_move:           // 4. 출하대기장이동등록(적치장이동)
                        if (start_grant("S14")) {
                            ...
                            Intent intent = TGSClass.ChangeView(getPackageName(), S14_HDR_Activity.class.getSimpleName());
                    ...
                    case R.id.btn_shipment_location_move_query:     // 5. 출하대기장이동현황조회
                        if (start_grant("S15")) {
                            Intent intent = TGSClass.ChangeView(getPackageName(), S15_QUERY_Activity.class.getSimpleName());
                    ...
                    case R.id.btn_shipment_registration_location:   // 6. 적치장재고출하등록(출하등록)
                        if (start_grant("S16")) {
                            Intent intent = TGSClass.ChangeView(getPackageName(), S16_HDR_Activity.class.getSimpleName());
                    ...
                    case R.id.btn_shipment_location_goods_status:   // 7. 출하대기장재고현황조회
                        if (start_grant("S17")) {
                            Intent intent = TGSClass.ChangeView(getPackageName(), S17_HDR_Activity.class.getSimpleName());
        ...
        try {
            ...
            if (ADMIN_CHK.equals("N") && W_OUT.equals("N")) {
                if (MenuID.equals("S11") && S11.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "출하요청현황 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("S12") && S12.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "출하등록 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("S13") && S13.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "출하현황 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("S14") && S14.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "출하대기장 이동등록(적치장 이동) 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("S15") && S15.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "출하대기장 이동현황조회 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("S16") && S16.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "적치장재고 출하등록(출하등록) 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("S17") && S17.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "출하대기장 재고현황조회  메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                }
            }
            return true;
