package com.example.EM_KOREA.myapplication.M30;
...
public class M31_HDR_Activity extends BaseActivity {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if ((event.getAction() == KeyEvent.ACTION_DOWN) && keyCode == KeyEvent.KEYCODE_ENTER) {
                    ...
                    if (dlv_no_data.equals("")) {
                        TGSClass.AlterMessage(getApplicationContext(), "조회조건 [거래명세서]는 필수입력사항입니다.");
                    } else {
                        start();
                    }
                    return true;
    ...
    private void start() {
        ...
        if (sJson.equals("[]")) {
            TGSClass.AlterMessage(getApplicationContext(), "거래명세서 정보가 없습니다.");
            return;
        } else {
            SET_TEXT_DLV_INFO();
        }
    ...
    public boolean import_customs_chk(final String dlv_no_data) {
        if (dlv_no_data.equals("")) {
            TGSClass.AlterMessage(getApplicationContext(), "입하대상을 검색하세요.");
            return false;
        } else {
            dbQuery_IMPORT_CUSTOMS_CHK_PO_TYPE_CD(dlv_no_data);

            if (sJson_po_type_cd.equals("[]")) {
                TGSClass.AlterMessage(getApplicationContext(), dlv_no_data + " 거래명세서 번호에 해당하는 발주타입(PO_TYPE_CD)가 없습니다.");
                return false;
            } else {
                SELECT_PO_TYPE_CD();
                ...
                if (sPO_TYPE_CD.equals("PO")) {
                    ...
                    if (sJson_uniscm.equals("[]")) {
                        AlertDialog.Builder builder = new AlertDialog.Builder(M31_HDR_Activity.this);
                        builder.setTitle("입하등록")
