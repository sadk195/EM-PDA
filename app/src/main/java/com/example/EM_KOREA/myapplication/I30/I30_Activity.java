...
public class I30_Activity extends BaseActivity {
    ...
    private Button btn_prod_shipment_status, btn_prod_location_move, btn_prod_location_move_query, btn_prod_location_goods_status,btn_prod_multi_order;
    private Button btn_menu;

    //== Grant 관련 변수 ==//
    private String ADMIN_CHK = "N", W_MOVE = "N", I31 = "N", I32 = "N", I33 = "N";
    private String I34 = "N", I35 = "N", I36 = "N", I37 = "N", I38 = "N",I39 = "N";     //Grant
    ...
    private final int I35_HDR_REQUEST_CODE = 5, I36_QUERY_REQUEST_CODE = 6, I37_HDR_REQUEST_CODE = 7, I38_HDR_REQUEST_CODE = 8, I39_HDR_REQUEST_CODE = 9;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_i30_sub_menu);
    ...
    private void initializeView() {
        ...
        //== ID값 바인딩 ==//
        btn_prod_release_registration           = (Button) findViewById(R.id.btn_prod_release_registration);            // 1. 생산출고 등록
        btn_out_of_plan                         = (Button) findViewById(R.id.btn_out_of_plan);                          // 2. 계획 외 출고양산용)
        btn_reservation_status                  = (Button) findViewById(R.id.btn_reservation_status);                   // 3. 자재예약현황
        btn_prod_shipment_status                = (Button) findViewById(R.id.btn_prod_shipment_status);                 // 4. 생산출고 현황조회
        btn_prod_location_move                  = (Button) findViewById(R.id.btn_prod_location_move);                   // 5. 생산출고 대기장이동(적치장이동)
        btn_prod_location_move_query            = (Button) findViewById(R.id.btn_prod_location_move_query);             // 6. 생산출고 대기장 이동 현황 조회
        btn_prod_release_registration_location  = (Button) findViewById(R.id.btn_prod_release_registration_location);   // 7. 적치장 재고 생산출고등록(생산출고등록)
        btn_prod_location_goods_status          = (Button) findViewById(R.id.btn_prod_location_goods_status);           // 8. 생산출고 대기장 재고 이동 현황 조회
        btn_prod_multi_order                    = (Button) findViewById(R.id.btn_prod_multi_order);                     // 9. 생산출고 등록(multi 오더) -2023-09-18 박준하 추가
    ...
    private void initializeListener() {
        View.OnClickListener clickListener = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (v == btn_menu) {        // 메뉴 버튼
                    ...
                    // 현재 Activity 종료
                    finish();
                } else if (v == btn_prod_release_registration) {            // 1. 생산출고등록 버튼
                ...
                else if (v == btn_prod_multi_order) {           // 9. 생산출고 등록(multi 오더)
                    if(start_grant("I39") == true) {
                        Intent intent = TGSClass.ChangeView(getPackageName(), I39_HDR_Activity.class);
                        intent.putExtra("MENU_ID", "I39");
                        intent.putExtra("MENU_REMARK", vMenuRemark);
                        intent.putExtra("START_COMMAND", vStartCommand);
                        startActivityForResult(intent, I39_HDR_REQUEST_CODE);
                    }
                }
            }
        };
        ...
        btn_prod_multi_order.setOnClickListener(clickListener);                     // 9. 생산출고 등록(multi 오더)
    ...
    private boolean start_grant(final String MenuID) {
        try {
            ...
            for (int idx = 0; idx < ja.length(); idx++) {
                ...
                I39 = LEVEL_CD.equals("I39") || I39.equals("Y") ? "Y" : "N";

            }

            if (ADMIN_CHK.equals("N") && W_MOVE.equals("N")) {
                if (MenuID.equals("I31") && I31.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "생산출고 등록 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("I34") && I34.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "생산 출고현황 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("I35") && I35.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "생산출고 대기장 이동(적치장이동) 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("I36") && I36.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "생산출고대기장 이동내역조회 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("I37") && I37.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "적치장 재고 생산출고등록(생산출고등록) 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                } else if (MenuID.equals("I38") && I38.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "생산출고대기장 재고현황조회 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                }
                else if (MenuID.equals("I39") && I38.equals("N")) {
                    TGSClass.AlterMessage(getApplicationContext(), "생산출고 등록(MULTI 오더) 메뉴에 대한 접속 권한이 없습니다.");
                    return false;
                }
            }
            return true;
