...
public class I39_DEL_Activity extends BaseActivity {
    ...
    private final int I39_DEL_REQUEST_CODE = 0;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_i39_del);
    ...
    private void start() {
        ...
        try {
            ...
            I39_DEL_ListViewAdapter listViewAdapter = new I39_DEL_ListViewAdapter(); //

            for (int idx = 0; idx < ja.length(); idx++) {
                JSONObject jObject = ja.getJSONObject(idx);

                I39_DEL item = new I39_DEL();
            ...
            listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView parent, View v, int position, long id) {
                    I39_DEL vItem = (I39_DEL) parent.getItemAtPosition(position);

                    Intent intent = TGSClass.ChangeView(getPackageName(), I39_DEL_Activity.class.getSimpleName());

                    intent.putExtra("HDR", vItem);  // 선택한 리스트를 파라메터로 다음페이지에 넘김.

                    startActivityForResult(intent, I39_DEL_REQUEST_CODE);
                }
            });
