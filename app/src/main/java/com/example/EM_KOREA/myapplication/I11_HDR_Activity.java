    ...
    private void initializeListener() {
        btn_query.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                /* 적치장 필수 값이 필요없을 것 같아서 주석처리
                if (txt_scan_location.getText().toString().isEmpty()) {
                    TGSClass.AlterMessage(getApplicationContext(), "적치장을 입력 해주세요.");
                    txt_scan_location.requestFocus();
                } else {
                    start();
                    //TGSClass.AlterMessage(getApplicationContext(), sJson);
                }

                if (!txt_scan_item_st.equals("")) {
                    if (txt_scan_item_st.contains(";")) {
                        int semicolon_index = txt_scan_item_st.indexOf(";");
                        String txt_scan_item_st_cut = txt_scan_location_st.substring(0, semicolon_index);
                        txt_scan_item.setText(txt_scan_item_st_cut);

                        start(txt_scan_location_st, txt_scan_item_st_cut);
                    } else {
                ...
                if ((keyEvent.getAction() == KeyEvent.ACTION_DOWN) && i == KeyEvent.KEYCODE_ENTER) {
                    ...
                    if (!txt_scan_item_st.equals("")) {
                        if (txt_scan_item_st.contains(";")) {
                            ...
                            //TGSClass.AlterMessage(getApplicationContext(), txt_scan_item_st_cut);
        ...
        try {
            ...
            listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView parent, View v, int position, long id) {
                    ...
                    Intent intent = TGSClass.ChangeView(getPackageName(), I11_DTL_Activity.class.getSimpleName());
