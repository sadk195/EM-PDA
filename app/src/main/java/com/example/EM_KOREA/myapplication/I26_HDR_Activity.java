...
public class I26_HDR_Activity extends BaseActivity {
    ...
    private void start() {
        ...
        if (sJson.equals("") || !TGSClass.isJsonData(sJson) || sJson.equals("[]")) {
            TGSClass.AlterMessage(getApplicationContext(), "출고이동요청 정보가 없습니다.");
            return;
        } else {
            setHDR();
            setDTL();
    ...
    private void setDTL() {
        try {
            ...
            TGSClass.AlterMessage(getApplicationContext(), ja.length() + " 건 조회가 되었습니다.");
        } catch (JSONException ex) {
            TGSClass.AlterMessage(this, ex.getMessage());
    ...
    private void save() {
        ...
        if (bl_send(strkey, "A")) {
            ...
            final AlertDialog.Builder alert = new AlertDialog.Builder(I26_HDR_Activity.this);
            alert.setTitle("출고이동 불가")
